...
import random
import numpy as np
from asociety.personality.ipipneo.reverse import IPIP_NEO_ITEMS_REVERSED_120
# These mappings are derived from the logic in internal_consistency_panel.py
# and the original ipip-neo-pi code.

REVERSED_QUESTIONS_120 = IPIP_NEO_ITEMS_REVERSED_120
"""
[
...
def _load_and_prepare_data(db_path):
    ...
    try:
        ...
        # For now, only 120 is fully supported here based on REVERSED_QUESTIONS_120
        if num_questions == 120:
            reversed_list = REVERSED_QUESTIONS_120
            for q_num in reversed_list:
                col_name = f'q{q_num}'
                if col_name in df.columns:
                    df[col_name] = 6 - df[col_name]
        else:
            # Placeholder for 300-item reverse scoring if needed
            pass
...
def perform_cfa(db_path: str):
    ...
    # Define the factors as independent (uncorrelated)
    # This is the key assumption for testing the Big Five model
