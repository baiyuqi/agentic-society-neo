...
此工具与"聚类分析"功能类似，但允许您**手动选择多个文件**进行聚类分析，而不是选择一个包含多个文件的目录。

您可以多次点击"添加文件..."按钮，选择您想要分析的特定数据库文件，然后对这些文件进行K-Means聚类分析。

### 如何使用

1.  点击"添加文件..."按钮，选择您想要分析的 `.db` 文件。
2.  重复步骤1，添加多个文件（至少需要2个文件）。
3.  点击"运行聚类"进行分析。
4.  分析完成后，可以查看图表和指标。
5.  点击"结果解析 (AI)"让AI帮助解读分析结果。
6.  点击"拷贝数据 (JSON)"将分析数据复制到剪贴板。
7.  点击"清除文件"可以清空已选择的文件列表。

### 如何解读结果

-   **调整兰德指数 (ARI)**: 衡量聚类结果与真实来源的一致性
-   **PCA 散点图**: 显示数据点在主成分空间的分布
-   **聚类中心距离**: 评估不同画像之间的分离程度

### 适用场景

- 当您只想分析特定几个文件，而不是整个目录时
- 当您想要比较不同来源的特定画像时
- 当您需要灵活选择分析对象时
""",
    'en': """
...
This tool is similar to the "Clustering Analysis" function but allows you to **manually select multiple files** for clustering analysis, rather than selecting a directory containing multiple files.

You can click the "Add File..." button multiple times to select specific database files you want to analyze, then perform K-Means clustering analysis on these files.

### How to Use

1.  Click the "Add File..." button to select the `.db` file you want to analyze.
2.  Repeat step 1 to add multiple files (at least 2 files required).
3.  Click "Run Clustering" to perform the analysis.
4.  After analysis, you can view the chart and metrics.
5.  Click "Analyze Results (AI)" to get AI interpretation of the results.
6.  Click "Copy Data (JSON)" to copy the analysis data to the clipboard.
7.  Click "Clear Files" to clear the selected file list.

### How to Interpret Results

-   **Adjusted Rand Index (ARI)**: Measures the agreement between clustering results and true sources
-   **PCA Scatter Plot**: Shows the distribution of data points in principal component space
-   **Cluster Centroid Distances**: Evaluates the separation between different profiles

### Use Cases

- When you only want to analyze specific files, not an entire directory
- When you want to compare specific profiles from different sources
- When you need flexible selection of analysis objects
"""
},
'single_density': {
    'zh': """
...
此工具用于在**单个图中绘制多个文件的马氏距离密度分布**，便于直观比较不同数据源的分布特征。

与"多数据源马氏距离对比分析"不同，此工具将所有选中的文件绘制在同一张图中，便于直接比较不同数据源的分布形状、集中程度和离散程度。

**关键特性**: 所有选中的数据集使用**共同的参考框架**进行计算，确保公平比较。
...
-   **变异系数**: 衡量数据的相对离散程度，值越大表示数据越分散
-   **峰度**: 衡量分布的尖锐程度，正值表示分布比正态分布更尖锐，负值表示更平坦

### 核心算法说明

所有选中的数据集首先被合并，计算一个**全局均值向量**和**全局协方差矩阵**。然后使用这个共同的参考框架来计算每个数据集中每个数据点的马氏距离。这种方法确保了所有数据集在相同的统计参考系下进行比较，避免了因数据集大小或分布差异导致的偏差。

### 适用场景

- 当您想要直观比较多个数据源的马氏距离分布时
- 当您需要分析不同画像的内部一致性差异时
- 当您想要观察数据分布的集中趋势和离散程度时
- 当您需要在公平的参考框架下比较不同数据集时
""",
    'en': """
### Function and Purpose

This tool is used to plot **Mahalanobis distance density distributions for multiple files in a single chart**, making it easy to visually compare distribution characteristics across different data sources.

Unlike "Multi-Source Mahalanobis Comparison Analysis", this tool plots all selected files in the same chart, facilitating direct comparison of distribution shapes, concentration levels, and dispersion across different data sources.

**Key Feature**: All selected datasets use a **common reference frame** for calculations, ensuring fair comparison.

### How to Use

1.  Click the "Add File..." button to select the `.db` file you want to analyze.
2.  Repeat step 1 to add multiple files (at least 1 file required).
3.  Click "Run Analysis" to perform the analysis.
4.  Use the "Switch to Histogram/Switch to Curves" button to toggle between view modes.
5.  After analysis, you can view the chart and statistical metrics.
6.  Click "Save to SVG" to save the chart as a vector image.
7.  Click "Clear Files" to clear the selected file list.

### How to Interpret Results

-   **Curve Mode (KDE)**: Shows smooth probability density curves for easy observation of distribution shapes
-   **Histogram Mode**: Shows traditional histograms for observing data distribution
-   **Variation Coefficient**: Measures relative dispersion of data, higher values indicate more scattered data
-   **Kurtosis**: Measures the sharpness of the distribution, positive values indicate sharper distribution than normal, negative values indicate flatter distribution

### Core Algorithm Explanation

All selected datasets are first combined to calculate a **global mean vector** and **global covariance matrix**. Then this common reference frame is used to calculate Mahalanobis distances for each data point in each dataset. This approach ensures all datasets are compared within the same statistical reference frame, avoiding biases due to dataset size or distribution differences.

### Use Cases

- When you want to visually compare Mahalanobis distance distributions across multiple data sources
- When you need to analyze internal consistency differences between different profiles
- When you want to observe concentration trends and dispersion of data distributions
- When you need to compare different datasets within a fair reference frame
"""
}
}